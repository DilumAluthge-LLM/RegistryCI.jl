...
function check_properties(result, props::TestProps)
    ...
    if props.diff_fence_count !== nothing
        if !check_fences(result, props.diff_fence_count, "diff")
            return false,
            "Expected $(props.diff_fence_count) backticks in diff fence in comment:\n\n$result"
        end
    end
    ...
    if props.sh_fence_count !== nothing
        if !check_fences(result, props.sh_fence_count, "sh")
            return false,
            "Expected $(props.sh_fence_count) backticks in sh fence for in comment:\n\n$result"
        end
    end
...
@testset "format_diff_stats" begin
    ...
    test_cases = [
        ...
        (
            ...
            shortstat=default_shortstat,
            props=TestProps(
                has_details=true, has_stat=true, diff_fence_count=4, sh_fence_count=3
            ),
        ),
        (
            ...
            shortstat=default_shortstat,
            props=TestProps(
                has_details=true, has_stat=true, diff_fence_count=3, sh_fence_count=5
            ),
        ),
    ...
    @testset "$(tc.name)" for tc in test_cases
        result = AutoMerge.format_diff_stats(
            tc.full_diff, tc.stat, tc.shortstat; old_tree_sha=old_sha, new_tree_sha=new_sha
        )
